...
import { INTERNAL_OBJECT, resolve } from './utils'
...
export default function array<T>(
  ...
  const proxy = new Proxy(
    [] as any as T[] & { [INTERNAL_OBJECT]: Y.Array<T> },
    {
      get(target, prop, receiver) {
        const key = toKey(prop)

        if (key === INTERNAL_OBJECT) {
          return arr
        }
        ...
        if (key === 'push') {
          return function () {
            arr.push(Array.from(arguments).map(resolve))
        ...
        if (key === 'splice') {
          return function () {
            ...
            transact(() => {
              ...
              arr.insert(start, items.map(resolve))
            })
        ...
        if (key === 'unshift') {
          return function () {
            arr.unshift(Array.from(arguments).map(resolve))

            return arr.length
          }
        }

        // TODO: implement these methods separately, reverse is broken and copyWithin, fill and sort might need some extra conversion / resolve logic
        if (
          key === 'copyWithin' ||
          key === 'fill' ||
          key === 'reverse' ||
